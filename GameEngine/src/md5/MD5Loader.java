...
public class MD5Loader {
	 ...
	 *            The MD5 Animation
	 * @param defaultColour
	 *            Default colour to use if there are no textures
	 * @return
	 * @throws Exception
	 */
	...
	private static List<Matrix4f> calcInJointMatrices(MD5Model md5Model) {
        ...
        for (MD5JointInfo.MD5JointData joint : joints) {
            ...
            Matrix4f mat = new Matrix4f().translate(joint.getPosition());
                   mat = Matrix4f.rotate(joint.getOrientation().w, new Vector3f(joint.getOrientation().x,  joint.getOrientation().y,  joint.getOrientation().z), mat, null);
                   mat = Matrix4f.invert(mat, mat);
            result.add(mat);
        }
        return result;
    }
	...
	private static Mesh generateMesh(MD5Model md5Model, MD5Mesh md5Mesh) {
		...
		for (MD5Mesh.MD5Vertex md5Vertex : md5Vertices) {
			...
			for (int i = startWeight; i < startWeight + numWeights; i++) {
				...
				org.joml.Vector3f rotatedPosJ = new org.joml.Vector3f(weight.getPosition().x, weight.getPosition().y, weight.getPosition().y).rotate(new org.joml.Quaternionf(joint.getOrientation().x, joint.getOrientation().y, joint.getOrientation().z, joint.getOrientation().w));
				Vector3f rotatedPos = new Vector3f(rotatedPosJ.x, rotatedPosJ.y, rotatedPosJ.z);
				Vector3f acumPos = Vector3f.add(new Vector3f(joint.getPosition()), rotatedPos, null);
				acumPos = new Vector3f(acumPos.x * weight.getBias(), acumPos.y * weight.getBias(), acumPos.z * weight.getBias());
		...
		for (MD5Mesh.MD5Triangle tri : md5Mesh.getTriangles()) {
			...
			v0.normal = Vector3f.add(v0.normal, normal, null);
			v0.normal.normalise();
			v1.normal = Vector3f.add(v0.normal, normal, null);
			v1.normal.normalise();
			v2.normal = Vector3f.add(v0.normal, normal, null);
			v2.normal.normalise();
		}
	...
	private static AnimatedFrame processAnimationFrame(MD5Model md5Model, MD5AnimModel animModel, MD5Frame frame,
		...
		for (int i = 0; i < numJoints; i++) {
			...
			Vector4f orientation = baseFrameData.getOrientation();
			...
			Matrix4f rotationMat = Matrix4f.rotate(orientation.w,
					new Vector3f(orientation.x, orientation.y, orientation.z), new Matrix4f(), null);
			Matrix4f jointMat = Matrix4f.mul(rotationMat, translateMat, null);

			// Joint position is relative to joint's parent index position. Use
			// parent matrices
			...
			if (joint.getParentIndex() > -1) {
				...
				jointMat = Matrix4f.mul(new Matrix4f(parentMatrix), jointMat, null);
			}
